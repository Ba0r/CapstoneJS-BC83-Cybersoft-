...
const renderAdminTable = (products) => {
  tableBody.innerHTML = products.map(p => `
    <tr class="text-center">
      ...
      <td>
        <button class="btn btn-sm btn-info" onclick="window.handleEdit('${p.id}')">Sửa</button>
        <button class="btn btn-sm btn-danger" onclick="window.handleDelete('${p.id}')">Xóa</button>
      </td>
    </tr>
  `).join('');
};
...
const filterAndSort = () => {
    ...
    if (searchTerm) listToRender = listToRender.filter(p => p.name.toLowerCase().includes(searchTerm));
